...
Mantém texto aprovado já existente ("O impossível na rua.") e os 12 vídeos FOOH como conteúdo. **O "carrossel 3D contínuo" mencionado originalmente aqui está SUPERSEDIDO — não é mais a estrutura correta.** A estrutura definitiva de exibição dos vídeos é a spec de coverflow com pin-scroll da seção 12.10/13.11: 3 vídeos por vez (um em destaque, dois nas laterais menores/apagados), avançando um de cada vez conforme o scroll, seção pinada até acabar — nunca um carrossel contínuo/automático mostrando tudo de uma vez.
...
**Referência confirmada 31/07/2026:** Ericson mandou [motion.dev/examples/vue-carousel-coverflow](https://motion.dev/examples/vue-carousel-coverflow) — nome da técnica bate com o que já estava especificado acima (efeito "coverflow": item central em destaque, itens vizinhos menores/apagados nas laterais). Não consegui capturar screenshot funcional do exemplo ao vivo (página com renderização pesada — mesma limitação técnica já registrada antes com lusion.co; confirmei via DOM que o carrossel tem 9 fotos carregadas, mas não validei visualmente o comportamento exato de transição). Ericson confirmou por texto a mecânica de scroll, que fecha o comportamento já especificado acima: **ao entrar na seção, o scroll trava (pin) e cada "tick" de rolagem avança um item do carrossel; ao chegar no último item, o scroll para de ser capturado pelo carrossel e a página volta a rolar normalmente até o final.** Usar essa página do Motion como referência de implementação técnica (é Vue, mas a técnica de coverflow com Motion/GSAP é portável pro nosso stack vanilla JS + GSAP ScrollTrigger).
...
**Correto (confirmado por análise frame-a-frame do vídeo "Lusion - About Us" em dois níveis de zoom, incluindo um crop enviado por Ericson mostrando as letras "AWARD" de perto):**
- O texto (ex.: "AWARD", "AWARDS", "CREATIVE STUDIO") é gigante, estilo watermark de fundo, mas **renderizado só com o CONTORNO (stroke) da fonte, sem preenchimento** — por padrão as letras são quase invisíveis, um traço fino cinza-escuro sobre o fundo preto, não um texto cinza sólido.
- **O efeito acontece DENTRO do contorno das letras, como se o texto fosse uma máscara/stencil**: uma distorção líquida colorida (fluid warp, canais RGB com leve separação — laranja/vermelho e azul se misturando de forma orgânica, não um simples ghost duplicado) aparece recortada exatamente na forma das letras, na posição onde o mouse está/passou. Fora da forma das letras não aparece nada — o efeito nunca "vaza" pro fundo preto ao redor.
- **A intensidade do efeito é proporcional à VELOCIDADE do mouse, não à posição/proximidade:** Ericson confirmou isso explicitamente — *"é um efeito cromático medido pela velocidade do mouse, quanto mais rápido mais intenso é o efeito, devagar é quase zero o efeito."* Ou seja: mouse parado ou se movendo devagar sobre o texto = efeito quase imperceptível; mouse se movendo rápido = distorção forte e colorida, com um leve rastro/trail (a cor "escorre" um pouco atrás do movimento, não é instantâneo/sem inércia).
- Tecnicamente: texto em stroke-only (SVG `text` com `fill: none; stroke: [cor escura]` ou `-webkit-text-stroke`, ou canvas), com uma camada de distorção (shader WebGL/Three.js com `feDisplacementMap`+ separação de canais, ou canvas 2D com ruído/fluid-warp) **clipada pela forma do texto** (`clip-path`, `mask`, ou renderizada dentro do próprio canvas/SVG do texto) e cuja intensidade (força do displacement + saturação/quantidade de separação RGB) é controlada por um cálculo de velocidade do mouse (delta de posição / delta de tempo entre frames), com decaimento suave (a intensidade não cai a zero instantaneamente quando o mouse para — some gradualmente, dando o efeito de rastro).
...
### 13.8 "A Forja" — refinamento adicional sobre a 13.6, com esboço do Ericson (31/07/2026)

Ericson viu o resultado do Bloco B e reforçou que o layout definitivo (13.6) foi bem implementado (banner, split, pin, última imagem com fade+botão, reversibilidade), mas fez três ajustes finos, incluindo um esboço desenhado à mão sobre um screenshot do build atual:

- **Banner (imagem da marreta no topo): aumentar o tamanho** — pedido já feito antes (11.7/12.8) e ainda não aplicado o suficiente; motivo explícito agora: o banner precisa ser maior pra mostrar melhor o render da marreta (hoje corta demais a peça).
- **Fade do banner: mais suave, e na direção de baixo pra cima** — não é mais "reduzir a intensidade" genericamente (11.7); é um gradiente vertical específico, mais forte/escuro na base do banner (onde encontra o resto da seção) suavizando pra cima, em vez do fade atual que apaga a imagem de forma mais brusca.
- **Bloco de imagem da galeria: precisa cobrir praticamente todo o espaço da esquerda, sem vão vazio entre a imagem e o início do texto.** Hoje a imagem fica colada só na borda esquerda da tela, deixando um espaço morto entre ela e a coluna de texto — o esboço do Ericson mostra a imagem da galeria ("IMAGEM GALERIA") ocupando quase toda a largura esquerda da seção, até encostar perto de onde a coluna de texto começa. Isso não contradiz a foto pequena dele ao lado do nome (já especificada na 13.6) — essa continua pequena, junto ao nome; é só o bloco grande de imagens da galeria que precisa se expandir pra preencher o espaço.

### 13.9 PENDÊNCIA — bibliotecas de efeito que Ericson quer reutilizar

Ericson comentou que os efeitos ainda estão "muito simples" e quer buscar inspiração em bibliotecas que ele já citou antes nesta conversa, mas não especificou quais nesta mensagem — disse que vai reenviar as referências diretamente. **Não presumir quais bibliotecas são; aguardar ele enviar antes de especificar qualquer nova dependência técnica no documento.**

**Primeira referência recebida (31/07/2026):** [motion.dev/examples/react-cursor-custom-content](https://motion.dev/examples/react-cursor-custom-content) — ver 13.10 abaixo, já incorporada à spec da galeria de "A Forja".

### 13.10 "A Forja" — galeria ganha cursor customizado com conteúdo ("View gallery"), referência testada ao vivo (31/07/2026)

Ericson mandou o link [motion.dev/examples/react-cursor-custom-content](https://motion.dev/examples/react-cursor-custom-content) e pediu explicitamente: *"essa daqui para colocar sob a galeria seria perfeito!"* Testei o exemplo ao vivo (hover real, não só descrição) pra confirmar o comportamento exato antes de especificar:

- É o componente **"Cursor" do Motion+ (motion.dev/docs/cursor)**, feature paga da biblioteca Motion (ex-Framer Motion) — não é grátis, então Claude Code precisa reimplementar o comportamento sem depender do pacote pago, salvo se Ericson tiver uma licença Motion+ (perguntar antes de assumir).
- **Comportamento confirmado ao vivo:** ao passar o mouse sobre a imagem (no exemplo, uma foto do grid), o cursor padrão SOME e é substituído por uma pílula (pill) rosa/magenta sólida com o texto **"View gallery"** dentro — a pílula segue a posição do mouse em tempo real, com um leve delay/suavização de mola (spring), não gruda instantaneamente na posição exata do ponteiro, dá uma sensação de "flutuar atrás" do cursor.
- Fora da área da imagem, o cursor volta ao normal (no nosso caso, volta a ser o cursor martelinho — ver regra global da 13.5).

**Aplicação em pixelforgestudio.com.br — substitui/complementa a mecânica de botão da 13.6:**
- Ao passar o mouse sobre o bloco de imagens da galeria em "A Forja" (qualquer imagem da sequência, não só a última), o cursor customizado (martelinho) é substituído por essa pílula com o texto **"Ver galeria"** (ou "Ver mais", conforme já sugerido na 12.8), seguindo o mouse com suavização de mola.
- Isso substitui a ideia de um botão fixo sobreposto só na última imagem (13.6) — em vez de um botão estático, a affordance de clique é o próprio cursor customizado, disponível durante toda a passagem das imagens da galeria, não só no final. O fade escurecido na última imagem (13.6) continua valendo como reforço visual de "isso é clicável", mas o mecanismo de clique/CTA agora é a área inteira da galeria com esse cursor customizado por cima.
- Tecnicamente (sem depender do pacote pago Motion+): implementável com um `div` fixo posicionado via `transform: translate()` seguindo `mousemove`, com interpolação de mola (GSAP `quickTo`, ou lerp manual, ou `Framer Motion`/`motion` grátis com `useSpring` — a parte paga é só o componente pronto `Cursor`, a técnica de mola em si é replicável com a lib gratuita ou até vanilla JS), escondido fora da área da galeria (`opacity`/`pointer-events` controlados por `mouseenter`/`mouseleave` no container da galeria).

### 13.11 FOOH — segunda referência, ticker/marquee de texto no fundo (31/07/2026)

Ericson mandou uma segunda opção pra seção FOOH: [motion.dev/examples/react-ticker-scroll](https://motion.dev/examples/react-ticker-scroll), como alternativa (ou complemento — "ou essa aqui") à spec de coverflow já registrada na 12.10, e depois confirmou de novo, direto: *"este é o efeito que quero na sessão FOOH, efeito ticker scroll."*

**Confirmado visualmente ao vivo (31/07/2026, com zoom preciso nas coordenadas certas do DOM — a primeira tentativa tinha falhado por mirar a região errada):**
- É uma faixa de texto (ticker/marquee) em fonte grande (~72-86px), só com contorno fino claro (sem preenchimento — outline puro, como as letras do efeito de distorção da 13.1), opacidade baixa (~50%), que se repete **encostada, sem espaço, de ponta a ponta da tela** (ex.: "Perfect Dark Perfect Dark Perfect Dark..." colado um no outro, sem gap).
- No exemplo, cada imagem (capa de jogo) fica ensanduichada entre duas faixas desse ticker — uma faixa em cima, uma embaixo — repetindo o nome daquele item específico. Ao rolar, a faixa de cima e a de baixo se movem horizontalmente (efeito de marquee contínuo), e a próxima imagem/ticker entra conforme o scroll avança.
- Reforça a leitura técnica: nome do exemplo é literalmente "Ticker: Scroll", i.e., o movimento horizontal do ticker é vinculado ao scroll (não é só autoplay constante — a página rolando dá "combustível" pro movimento, no mesmo espírito velocity-driven já confirmado no efeito de distorção da 13.1).

**Correção do próprio Ericson logo em seguida** ("eu devo ter falado errado"): o layout NÃO muda — é o mesmo split 50/50 que já existia (5.2/12.10). **O texto (bloco "O impossível na rua.") continua na metade esquerda, exatamente como já era. O carrossel/coverflow de vídeos (spec da 12.10) ocupa a metade direita**, também como já era. Não há mudança de layout aqui, só a adição do ticker de fundo abaixo.

Aplicação em pixelforgestudio.com.br:
- Layout confirmado, sem mudança: texto à esquerda (metade da tela), coverflow de vídeos à direita (metade da tela) — igual à spec já existente em 5.2/12.10.
- **Correção final do Ericson sobre a quantidade de faixas (31/07/2026):** não é uma linha só — é **VÁRIAS linhas empilhadas, preenchendo todo o fundo da seção verticalmente**, cada uma repetindo "* FAKE OUT OF HOME *" colado sem espaço (mesmo estilo watermark: fonte grande, só contorno, opacidade baixa). Suas palavras: *"eu queria tipo que ficasse atrás várias vezes repetindo, não somente uma linha, e ao dar scroll uma linha corre pra esquerda e outra pra direita sucessivamente."*
- **Direção alternada por linha:** ao rolar a página, as linhas se movem horizontalmente, mas alternando sentido — uma linha corre pra esquerda, a linha seguinte (abaixo) corre pra direita, a próxima de novo pra esquerda, e assim sucessivamente (padrão "zebra" de direção, comum em murais de texto tipo marquee empilhado). Velocidade/deslocamento vinculado ao scroll, no mesmo espírito velocity-driven já confirmado na 13.1.
- Preenche o fundo inteiro da seção (atrás de todo o conteúdo — texto à esquerda e coverflow à direita), não é mais "uma faixa" isolada — é um mural de várias linhas cobrindo toda a altura da seção.
- Tecnicamente: cada linha é um container `overflow: hidden` com o texto duplicado internamente (técnica padrão de marquee infinito), transladado via `transform: translateX()` — linhas de índice par com translateX negativo (esquerda) e ímpar com translateX positivo (direita), ambos atualizados proporcionalmente ao delta de scroll (GSAP ScrollTrigger `onUpdate` ou `scrub`, seguindo o mesmo padrão de scroll-linking já usado em outras seções do site).

**Reforço final do Ericson (31/07/2026):** confirmou de novo que a metade direita usa especificamente o efeito da referência de coverflow (12.10/motion.dev/examples/vue-carousel-coverflow) — **NÃO é mais um carrossel contínuo mostrando todos os vídeos de uma vez** (esse era o comportamento antigo, agora oficialmente substituído — ver correção na 5.2). É o mecanismo de pin-scroll: 3 vídeos visíveis por vez (um em destaque central, dois menores/apagados nas laterais), avançando um por vez conforme o usuário rola, seção presa até o último vídeo passar. Não implementar um carrossel automático/contínuo em nenhuma versão dessa seção.

### 13.12 Seção "IA + CGI" — troca de imagem com transição tipo "reorder", referência testada ao vivo (31/07/2026)

Ericson mandou [motion.dev/examples/vue-reorder-items](https://motion.dev/examples/vue-reorder-items) pra seção "IA + CGI" (spec de conteúdo/copy na 5.3, spec de banco de imagens alternando na 12.11): *"ao alternar muda a foto, sabe, mas ter algo dinâmico."*

Testei ao vivo (arrastei um item pra confirmar o comportamento): é o padrão **"Reorder" do Framer Motion/Motion** — uma grade de itens (no exemplo, 4 quadrados coloridos) onde, ao reordenar, os itens **fazem uma animação de reposicionamento fluida tipo FLIP** (cada item anima suavemente da posição antiga pra nova, com spring/easing, podendo se sobrepor brevemente durante a troca) em vez de simplesmente saltar/cortar pra nova posição.

**Aplicação em pixelforgestudio.com.br:** a seção "IA + CGI" já tem um quadro fixo com 4 posições de imagem, alternando periodicamente dentro de um banco maior (~20 imagens, spec da 12.11). Em vez da troca ser um corte seco ou um crossfade simples, aplicar essa mecânica de reposicionamento animado: quando uma imagem é substituída, ao invés de só trocar no lugar, o conjunto de 4 imagens reorganiza suas posições dentro do quadro com uma transição de movimento (a nova imagem "assume" um slot enquanto as outras deslizam/ajustam posição com spring/easing), dando a sensação de um grid vivo se reorganizando — não é necessário ser literalmente arrastável pelo usuário (o exemplo original é drag-to-reorder manual; aqui a "reordenação" é automática/temporizada, mas usa a MESMA técnica de animação de layout fluida entre as 4 posições).
- Tecnicamente: GSAP com FLIP plugin (`gsap/Flip`) é o equivalente direto no nosso stack (vanilla JS + GSAP, já confirmado como stack de animação — seção 6), sem precisar da lib Motion/Vue do exemplo.

### 13.13 Efeito de distorção em "Serviços" — ainda errado no Bloco 3, três bugs específicos identificados (31/07/2026)

Ericson mandou print do resultado do Bloco A (correção da 13.1) e foi direto: *"tá muito feio como ele tá fazendo... a letra tá distorcida, já tá feio, bem ruim mesmo, não me agrada."* Analisei o print com cuidado (ele avisou pra considerar a marreta como a posição real do mouse — a setinha fina no meio do print é só o cursor do Claude tirando o screenshot, não o cursor do site). Três problemas distintos, todos precisam de correção:

1. **Texto gigante demais / cortado nas bordas, ilegível.** No print, só dá pra ver fragmentos de letras nas duas pontas da tela (um pedaço arredondado à esquerda, um "S" à direita) — o resto da palavra "Serviços" está fora da área visível. Está grande demais pro viewport. **Correção: ou reduzir a escala pra caber a palavra inteira confortavelmente na tela, ou repetir a palavra "Serviços" mais de uma vez lado a lado** (padrão watermark repetido, não uma palavra única gigante cortada) — qualquer uma das duas resolve, mas do jeito atual (uma palavra só, maior que o viewport) está errado.
2. **Mapeamento do hover/cursor está errado — o efeito aparece longe da posição real do mouse.** No print, o cursor (martelinho) está bem próximo da borda direita da tela; o efeito de distorção (a mancha roxa/magenta) está centralizado bem mais à esquerda, longe do cursor. **Bug de posicionamento: a coordenada usada pro efeito não está sincronizada com a posição real do cursor customizado** — conferir se o listener de mouse está pegando a posição correta (`clientX`/`clientY` vs. coordenadas do cursor customizado renderizado, que pode estar com um offset ou defasagem) e se o efeito está sendo desenhado no sistema de coordenadas certo (viewport vs. elemento vs. canvas com seu próprio offset).
3. **As letras em si estão se distorcendo/derretendo — isso está errado.** A 13.1 já especifica que a distorção acontece DENTRO da máscara da letra (a letra funciona como stencil), mas o contorno/forma da letra em si (o "esqueleto" da fonte) deve ficar **geometricamente estático o tempo todo** — só o conteúdo visual recortado dentro dela (a distorção líquida colorida) se move/deforma. No print, a letra parece estar fisicamente empenada/derretida, o que não é o efeito da referência (Lusion). **Reforçar: usar o texto como `clip-path`/`mask` fixo (não deformar o path da fonte), e aplicar o shader/distorção só dentro dessa máscara.**

Esses três pontos substituem/detalham ainda mais a spec da 13.1 — não é uma direção nova, é a mesma spec, com bugs de implementação apontados com precisão pra próxima tentativa.

**Referência de design da letra em si, print em alta resolução (31/07/2026):** Ericson mandou um crop nítido do fundo do Lusion (as palavras "AWARD"/"WINNING" sobrepostas) e disse: *"daqui é a referência perfeita de design das letras, tem que ser assim, da forma que está lá tá bem feio e amador ainda."* Isso não é só sobre a mecânica do efeito (já coberta na 13.1/13.13) — é sobre a QUALIDADE VISUAL da tipografia/estilo da letra:
- Fonte sans-serif geométrica, bem bold/grossa, maiúsculas, contorno fino e nítido (não um traço grosso ou borrado).
- O traço do contorno é sutil — um cinza bem escuro/quase invisível contra o fundo preto, só perceptível de perto ou com o efeito de distorção ativo por cima.
- As letras ficam justapostas de forma bem compacta (kerning apertado, quase se tocando), reforçando a leitura como uma peça gráfica única de fundo, não como um título comum espaçado.
- A distorção colorida, quando ativa, aparece como **rastros/estrias diagonais de cor** (não uma mancha/blob uniforme) — faixas onde laranja/vermelho se misturam com azul/ciano de forma orgânica, como um líquido ou tinta escorrendo na diagonal, sempre confinado dentro do contorno da letra.
- **Isso é o padrão de qualidade a bater** — o que está implementado atualmente (13.13) está "feio e amador" tanto na execução técnica (os 3 bugs já listados) quanto no acabamento tipográfico em si (peso da fonte, nitidez do contorno, qualidade do rastro de cor). Usar este print como referência visual direta na próxima tentativa, não só a descrição em texto.

### 13.14 Box/painel da seção "Serviços" — "vidro" precisa ser shader de vidro de verdade, não transparência CSS — SUBSTITUI a menção de "glassmorphism" da 11.6

Ericson viu o painel atual (o print da 13.13 mostra um box escuro simples com borda, tipo card comum) e corrigiu: quando ele fala "vidro" (11.6 mencionou "efeito de vidro (glassmorphism)"), **não é um painel meio transparente com blur de CSS — é um shader de vidro de verdade**, com refração/distorção real, do tipo que existe nas bibliotecas de Three.js. Ele mandou a referência técnica exata: [blog.olivierlarose.com/tutorials/3d-glass-effect](https://blog.olivierlarose.com/tutorials/3d-glass-effect).

**Li o tutorial e confirmei a técnica:**
- É Three.js (via React Three Fiber no tutorial, mas a técnica em si é Three.js puro) usando o material **`MeshTransmissionMaterial`** do pacote `@react-three/drei` — uma extensão do `MeshPhysicalMaterial` do Three.js que adiciona propriedades de vidro físico real: `transmission` (transparência óptica real, não opacity), `thickness` (espessura do material, afeta a refração), `roughness` (nitidez da refração — 0 = vidro liso/nítido), `ior` (índice de refração), `chromaticAberration` (separação sutil de cor nas bordas, como vidro/lente de verdade), e `backside` (renderiza a face de trás do vidro pra refração correta).
- No tutorial, o material é aplicado numa mesh 3D real (um toro/torus feito no Blender e exportado em `.glb`), com uma luz direcional e um `Environment` (mapa de ambiente HDRI) pra dar reflexos realistas — sem ambiente/luz, o vidro não tem o que refletir/refratar e fica sem graça.
- **Isso significa que o painel de Serviços precisa ser um objeto 3D renderizado de verdade (Three.js), não uma `div` com `backdrop-filter: blur()`.** É uma peça 3D com geometria própria (pode ser um plano/box simples, não precisa ser um toro) posicionada atrás/ao redor da lista de serviços, usando esse material de transmissão, dentro de uma cena Three.js com luz + environment map.
- Tecnicamente pro nosso stack (Three.js vanilla, sem React/R3F): `@react-three/drei` é um pacote React-only, então não dá pra importar direto — mas a base é o `THREE.MeshPhysicalMaterial` nativo do Three.js, que já suporta `transmission`, `thickness`, `ior` e `roughness` nativamente (sem precisar do drei). `chromaticAberration` é uma extensão específica do drei (shader customizado em cima do `MeshPhysicalMaterial`) — se não for viável reproduzir esse parâmetro específico, o vidro com só `transmission`+`thickness`+`ior`+`roughness` (sem aberração cromática) já entrega o essencial do efeito realista; aberração cromática é um "nice to have", não bloqueador.
- Precisa de um `Environment`/mapa de reflexo (HDRI, ou pelo menos um cubemap simples/gerado) pra o vidro ter o que refletir — sem isso o material fica "morto" visualmente.

### 13.15 Efeito de distorção — vídeo novo com antes/depois, correção definitiva do "stroke" (31/07/2026)

Ericson gravou um vídeo novo específico pra esse efeito: primeiros ~15s mostram o texto de fundo do Lusion parado, sem mouse, pra deixar claro como é o estado de repouso; depois disso, ele passa o mouse rápido e mostra o efeito real, líquido, "nada seco como ele está fazendo". Extraí frames específicos (repouso, início do efeito, efeito em força total) pra fechar a spec:

**Correção principal, com as palavras do Ericson:** *"o que ele não entendeu ainda é que não é uma palavra com um stroke GROSSO atrás, é um stroke suave e o efeito aparece mais forte no stroke."*

**Confirmado nos frames extraídos:**
- **Estado de repouso (0-15s, sem mouse):** a palavra "AWARD" (e "WINNING" atrás) está praticamente invisível — só uma sugestão muito sutil da forma da letra contra o fundo preto, sem contorno grosso nem visível de longe. **Isso já estava certo na 13.1/13.13 — reforçar que não pode virar um contorno grosso e sempre visível.**
- **Efeito em força total (mouse passou rápido por cima):** o preenchimento líquido (marmorizado, tons de azul/vermelho/ciano, textura tipo metal líquido ou tinta oleosa) cobre a letra inteira, MAS a cor mais viva/saturada (o "rim light" ciano e vermelho vibrante) se concentra bem na BORDA/CONTORNO de cada letra — o interior da letra tem uma versão mais escura/opaca/suave da mesma cor. É esse contraste (borda brilhante, interior mais discreto) que cria a leitura de "stroke" — não é um stroke desenhado à parte, é o próprio efeito de luz/cor que fica mais forte exatamente onde o contorno da letra está.
- Isso reforça e refina a 13.13 (ponto 3): o contorno da letra continua sendo uma máscara fixa (não deforma), mas o SHADER de distorção dentro dela precisa ter um gradiente de intensidade — mais forte/brilhante perto da borda da máscara, mais suave/escuro no centro — não uma cor uniforme espalhada por igual dentro da letra inteira.
- **As letras realmente ocupam de um lado ao outro da tela** — confirma que a escala gigante (13.13, ponto 1) está correta como direção, só a implementação atual que cortou errado.

Tecnicamente: no shader, isso é equivalente a um `smoothstep`/gradiente radial baseado na distância até a borda da máscara (SDF — signed distance field — do contorno da letra, ou um simples blur/glow da borda do `clip-path`/`mask` sobreposto por cima do preenchimento líquido), fazendo a intensidade de cor (saturação/brilho) cair conforme se afasta da borda pro centro da letra.

### 13.16 Nav — fundo muda de forma abrupta no scroll, precisa de transição suave

Bug novo reportado por Ericson: o nav (barra fixa no topo) fica preto **de forma abrupta** assim que o usuário começa a rolar — sem transição, um corte seco de transparente pra preto sólido. Palavras dele: *"quebra imersão completo logo no começo."* Como é literalmente a primeira coisa que o usuário vê ao interagir com o site (primeiro scroll, ainda no Hero), esse corte brusco compromete a primeira impressão inteira.

**Correção:** o fundo do nav precisa fazer uma transição suave (fade/crossfade animado, não um `if scrollY > X { background: black }` direto) entre o estado transparente (topo da página) e o estado com fundo sólido/escuro (depois de rolar). Implementar com GSAP (`ScrollTrigger` com `scrub`, ou uma interpolação de opacidade do background baseada no scroll, ex.: opacity 0 a 1 ao longo de uma pequena faixa de scroll — não instantâneo). Mesma lógica de suavidade já cobrada em outras partes do site (reversibilidade de scroll, 11.13) — a transição também precisa reverter suavemente se o usuário rolar de volta pro topo.

### 13.17 Efeito de distorção — legibilidade é o problema PRINCIPAL + física de água como analogia definitiva (31/07/2026)

Ericson mandou novo print do resultado mais recente: o painel de Serviços (agora já com bordas mais estreitas, mais perto do centro) cobre o meio da palavra "SERVIÇOS" de fundo, deixando só fragmentos de "S" visíveis nas duas pontas — **ilegível**. Nas palavras dele: *"não dá pra ler Serviços, tem que ser de uma ponta a outra... mas o principal é o tamanho, pq nem dá pra ler."*

**Reforça e prioriza o ponto 1 da 13.13 (escala/corte do texto) como o problema #1 a resolver antes de qualquer polimento do shader:** a palavra "SERVIÇOS" precisa ser legível de ponta a ponta da tela — calcular o tamanho da fonte/letter-spacing pra a palavra inteira caber exatamente na largura do viewport com todas as letras completas e legíveis (nem cortada faltando pedaço de letra, nem tão grande que vira fragmento irreconhecível). Isso é mais importante que o acabamento do shader (13.15) — sem isso funcionar, o resto do efeito não importa.

**Nova analogia física definitiva pro comportamento do efeito, com as palavras do Ericson:** *"o efeito tem que ser tipo líquido, fluido e contínuo. O mouse para e ele segue levemente um pouco mais adiante, como se fosse empurrado — como mexer com um dedo na água parada: vai criar ondas e movimento na água, e ao tirar o dedo ou parar, ainda vai ter um fluxo leve até parar de vez."*

Isso adiciona uma camada de física por cima do que já estava especificado (13.1: intensidade proporcional à velocidade do mouse, com decaimento suave):
